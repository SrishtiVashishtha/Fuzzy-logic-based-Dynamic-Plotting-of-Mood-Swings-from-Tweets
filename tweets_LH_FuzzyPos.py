...
for j in range(senlen):
    ...
    #VERY POSITIVE , NOT VERY POSITIVE , MORE OR LESS POSITIVE
...
#    print(mlpos)
    ...
    for i in range(len(lp)):
        t=pow(ln[i],2)
        s=1-t
        notveryneg.append(round(s,4))
#    print("\n Not Very Negative : ")
#    print(notveryneg)
    
# INTERSECTION OPEARATION  MORE OR LESS POS AND NOT VERY NEGATIVE
    
    inter=[]
    print("\n Sentence "+ str(j+1) +" More or less Positive and not Very Negative Score for each word in sentence :")    
    for word in range(len(lp)):
        if mlpos[word]<notveryneg[word]:
            inter.append(mlpos[word])
        else:
            inter.append(notveryneg[word])
    print(inter)
    inte=round(sum([word_score for word_score in inter]),4)
    global_intersection.append(inte)
    
print("\n Mood Swing FuzzyPositive Values of Tweets for all timestamps (30) is : \n")
print(global_intersection)
plt.plot(global_intersection)
plt.title("Mood Swings FuzzyPositive values while watching IndVsPak ICC World Cup (2017) Final Match")
plt.xlabel("Time Stamp")
plt.ylabel("Card_FuzzyPos_Value")
plt.show()
...
print("\n Mood Swing FuzzyPositive Values of Tweets when there is fixed timestamp (3) :")
print(global_intersection_timestamp_sum)

plt.plot(global_intersection_timestamp_sum)
plt.title("Mood Swings FuzzyPositive Values while watching IndVsPak ICC World Cup (2017) Final Match")
plt.xlabel("Time Stamps")
plt.ylabel("FuzzyPos_Value")
plt.show()

filename = 'FuzzyPos.pickle'
